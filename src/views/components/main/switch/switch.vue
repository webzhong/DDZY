...
export default {
    ...
    methods: {
        setTheme (themeFile) {
            let menuTheme = themeFile.substr(0, 1);		// ...主题颜色：'dark'、'light'
            let mainTheme = themeFile.substr(-1, 1);	// ...字体颜色：blue、green、yellow、red
            if (menuTheme === 'b') {
                // ...设置主题颜色:黑色菜单
            ...
            if (config.env.indexOf('dev') > -1) {
                stylePath = './src/views/components/main/switch/theme/';
            } else {
                stylePath = 'dist/';
            }
            if (mainTheme !== 'b') {
    ...
    created () {
        let path = '';
        if (config.env.indexOf('dev') > -1) {
            path = './src/views/components/main/switch/theme/';
        } else {
            path = 'dist/';
        }
        let name = sessionStorage.user;
        if (localStorage.theme) {
